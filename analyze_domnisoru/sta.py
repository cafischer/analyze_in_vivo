...
if __name__ == '__main__':
    ...
    param_list = ['Vm_ljpc', 'spiketimes', 'vel_100ms', 'fY_cm']
    ...
    for i, cell_id in enumerate(cell_ids):
        ...
        if in_field:
            spike_train = init_nan(len(v))  # for velocity thresholding to match in field idxs
            spike_train[AP_max_idxs] = range(len(AP_max_idxs))
            [spike_train], _ = threshold_by_velocity([spike_train], velocity, threshold=1)
            assert len(data['fY_cm']) == len(spike_train)  # make shure same velocity thresholding as Domnisoru
