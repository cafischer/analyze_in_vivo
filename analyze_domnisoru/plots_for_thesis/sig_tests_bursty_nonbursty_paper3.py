...
fraction_single = np.load(os.path.join(save_dir_n_spikes_in_burst, 'fraction_single_' + str(burst_ISI) + '.npy'))
...
n_cols = 6
outer = gridspec.GridSpec(2, n_cols + 1)

data = [fraction_burst, fraction_single, fraction_ISI_or_ISI_next_burst, vdiff_onset_start,  width_ISI_hist, peak_ISI_hist]
data_latuske = [fraction_burst_latuske, fraction_single_latuske, fraction_ISI_or_ISI_next_burst_latuske, None,
                width_ISI_hist_latuske, peak_ISI_hist_latuske]
...
sigmas = [0.01, 0.01, 0.01, 0.01, 1, 1]
letters = ['A', 'B', 'C', 'D', 'E', 'F']
